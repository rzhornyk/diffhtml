...
import { PATCH_TYPE, EMPTY } from '../util/types';
    ...
    while (true) {
        ...
        switch (patchType) {
            case PATCH_TYPE.SET_ATTRIBUTE: {
                ...
                const domNode = /** @type {HTMLElement} */ (createNode(vTree, ownerDocument, isSVG));
                if (!domNode) {
                    break;
                }
                const oldValue = domNode.getAttribute(name);
                const attributeChangedPromises = runTransitions('attributeChanged', vTree, name, oldValue, value);
                protectVTree(vTree);
            ...
            case PATCH_TYPE.REMOVE_ATTRIBUTE: {
                ...
                const oldValue = domNode.getAttribute(name);
                const attributeChangedPromises = runTransitions('attributeChanged', vTree, name, oldValue, null);
                protectVTree(vTree);
                if (attributeChangedPromises.length) {
                    Promise.all(attributeChangedPromises)
            ...
            case PATCH_TYPE.NODE_VALUE: {
                ...
                const isSVG = svgElements.has(vTree);
                i += 4;
                const domNode = /** @type {Text} */ (createNode(vTree, ownerDocument, isSVG));
                if (!domNode) {
                    break;
                }
                protectVTree(vTree);
            ...
            case PATCH_TYPE.INSERT_BEFORE: {
                ...
                const refNode = refTree && /** @type {HTMLElement} */ (createNode(refTree, ownerDocument, isSVG));
                const newNode = /** @type {HTMLElement} */ (createNode(newTree, ownerDocument, isSVG));
            ...
            case PATCH_TYPE.REPLACE_CHILD: {
                ...
                const newDomNode = /** @type {HTMLElement} */ (createNode(newTree, ownerDocument, isSVG));
                // Patching without an existing DOM Node is a mistake, so we should not
                // attempt to do anything in this case.
                ...
                const hasAttached = (_a = TransitionCache.get('attached')) === null || _a === void 0 ? void 0 : _a.size;
                const hasDetached = (_b = TransitionCache.get('detached')) === null || _b === void 0 ? void 0 : _b.size;
                const hasReplaced = (_c = TransitionCache.get('replaced')) === null || _c === void 0 ? void 0 : _c.size;
                ...
                const replacedPromises = runTransitions('replaced', oldTree, newDomNode);
                const allPromises = [
                    ...attachedPromises,
                    ...detachedPromises,
                    ...replacedPromises,
