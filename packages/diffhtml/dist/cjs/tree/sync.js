...
function syncTree(oldTree, newTree, patches = [], state = {}, attributesOnly = false) {
  if (!oldTree) oldTree =
  /** @type {VTree} */
  _types.EMPTY.OBJ;
  if (!newTree) newTree =
  ...
  const isEmpty = oldTree === _types.EMPTY.OBJ || attributesOnly; // Check for SVG in parent.

  const isSVG = newNodeName === 'svg' || svgElements.has(
  /** @type {VTree} */
  newTree);
  let shortCircuit = null; // Invoke the SyncTree hooks which allow middleware to do their own custom
  ...
  if (_caches.SyncTreeHookCache.size) {
    _caches.SyncTreeHookCache.forEach(fn => {
      ...
      const entry = fn(oldTree, newTree); // If the value returned matches the original element, then short circuit
  ...
  if (newNodeName === textName) {
    if (oldNodeName === textName && oldTree.nodeValue !== newTree.nodeValue) {
      ...
      // `Element#nodeValue`.
      patches.push(_types.PATCH_TYPE.NODE_VALUE, oldTree, newTree.nodeValue, oldTree.nodeValue);
      oldTree.nodeValue = newTree.nodeValue;
      return patches;
    } // Ensure new text nodes have decoded entities.
    else if (isEmpty) {
        patches.push(_types.PATCH_TYPE.NODE_VALUE, newTree, newTree.nodeValue, null);
        return patches;
      }
  }

  const newChildNodes = newTree.childNodes || []; // Seek out attribute changes first, but only from element Nodes.

  if (newTree.nodeType === 1) {
    const oldAttributes = isEmpty ? _types.EMPTY.OBJ : oldTree.attributes;
    ...
    for (let key in newAttributes) {
      ...
      (!oldTree || oldTree.nodeName !== 'script') && newTree.nodeName === 'script' && key === 'type') {
        continue;
      }

      patches.push(_types.PATCH_TYPE.SET_ATTRIBUTE, isEmpty ? newTree : oldTree, key, value);
    } // Search for removals.
    ...
    if (!isEmpty) {
      for (let key in oldAttributes) {
        ...
        patches.push(_types.PATCH_TYPE.REMOVE_ATTRIBUTE, oldTree, key);
  ...
  const oldChildNodes = oldTree.childNodes; // Used when skipping over nodes during syncTreeHook.
  ...
  for (let i = 0; i < maxLength; i++) {
    ...
    const newChildNode = newChildNodes[i]; // Check for SVG in child as well.
    ...
    if (!oldChildNode) {
      oldChildNodes.push(newChildNode); // Crawl this Node for any changes to apply.

      syncTree(null, newChildNode, patches, state, true);
      patches.push(_types.PATCH_TYPE.INSERT_BEFORE, oldTree, newChildNode, null);
      continue;
    }
    ...
    if (oldKey || newKey) {
      ...
      if (!oldInNew && !newInOld) {
        ...
        syncTree(null, newChildNode, patches, state, true);
        patches.push(_types.PATCH_TYPE.REPLACE_CHILD, newChildNode, oldChildNode);
        i = i - 1;
        continue;
      } else if (!oldInNew) {
        patches.push(_types.PATCH_TYPE.REMOVE_CHILD, oldChildNode);
      ...
      if (newKey !== oldKey) {
        ...
        patches.push(_types.PATCH_TYPE.INSERT_BEFORE, oldTree, optimalNewNode, oldChildNode);
    ...
    if (!sameType) {
      oldChildNodes[i] = newChildNode; // This only works if VTrees are identical.
  ...
  if (oldChildNodes.length !== newChildNodes.length) {
    for (let i = newChildNodes.length; i < oldChildNodes.length; i++) {
      patches.push(_types.PATCH_TYPE.REMOVE_CHILD, oldChildNodes[i]);
    }
