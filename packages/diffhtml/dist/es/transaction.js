...
import { VTree, ValidInput, Mount, Options, TransactionState, EMPTY } from "./util/types.js";
import { MiddlewareCache, StateCache, NodeCache } from "./util/caches.js";
import { gc } from "./util/memory.js";
import makeMeasure from "./util/make-measure.js";
import process from "./util/process.js";
import schedule from "./tasks/schedule.js";
import shouldUpdate from "./tasks/should-update.js";
import reconcileTrees from "./tasks/reconcile-trees.js";
import syncTrees from "./tasks/sync-trees.js";
import patchNode from "./tasks/patch-node.js";
import endAsPromise from "./tasks/end-as-promise.js";
...
export default class Transaction {
   ...
   */
    ...
    scriptsToExecute.forEach((type = EMPTY.STR, vTree) => {
      const oldNode =
      /** @type {HTMLElement} */
      NodeCache.get(vTree); // Reset the type value.
    ...
    /** @type {any} */
    domNode ? domNode.outerHTML : EMPTY.STR; // Only execute scripts if the configuration is set. By default this is set
    ...
    gc(); // Trigger all `onceEnded` callbacks, so that middleware can know the
