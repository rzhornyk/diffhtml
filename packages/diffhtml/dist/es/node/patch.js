...
import { runTransitions } from "../transition.js";
import { protectVTree, unprotectVTree } from "../util/memory.js";
import decodeEntities from "../util/decode-entities.js";
import { PATCH_TYPE, ValidNode, VTree, EMPTY } from "../util/types.js";
import { NodeCache, TransitionCache } from "../util/caches.js";
const {
  keys
} = Object;
      ...
      case PATCH_TYPE.INSERT_BEFORE:
        {
          ...
          let domNode =
          /** @type {HTMLElement} */
          NodeCache.get(vTree); // Patching without an existing DOM Node is a mistake, so we should not
      ...
      case PATCH_TYPE.REPLACE_CHILD:
        {
          ...
          protectVTree(newTree); // Check if there are any transitions, if none, then we can skip
          ...
          const hasReplaced = TransitionCache.get('replaced')?.size; // In the hot path, ensure we always use the fastest operation given
