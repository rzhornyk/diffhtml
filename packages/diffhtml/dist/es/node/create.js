import { NodeCache, CreateNodeHookCache } from "../util/caches.js";
import process from "../util/process.js";
import globalThis from "../util/global.js";
import { VTreeLike, VTree, ValidNode, EMPTY } from "../util/types.js";
...
export default function createNode(vTreeLike, ownerDocument = document, isSVG) {
  ...
  const existingNode = NodeCache.get(vTree); // If the DOM Node was already created, reuse the existing node. This is
  ...
  const validNode = domNode; // Add to the domNodes cache.

  NodeCache.set(vTree, validNode); // Append all the children into the domNode, making sure to run them
