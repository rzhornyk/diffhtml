import { MiddlewareCache, CreateTreeHookCache, CreateNodeHookCache, SyncTreeHookCache, ReleaseHookCache, ParseHookCache } from "./util/caches.js";
import process from "./util/process.js";
import { Middleware } from "./util/types.js";
...
export default function use(middleware) {
  ...
  isFunction && MiddlewareCache.add(
  /** @type {Function} */
  ...
  parseHook && ParseHookCache.add(parseHook); // The unsubscribe method for the middleware.

  return () => {
    // Remove this middleware from the internal cache. This will prevent it
    // from being invoked in the future.
    isFunction && MiddlewareCache.delete(
    /** @type {Function} */
