import { SyncTreeHookCache } from "../util/caches.js";
import process from "../util/process.js";
import { PATCH_TYPE, VTree, EMPTY, TransactionState } from "../util/types.js";
const {
  max
} = Math;
...
export default function syncTree(oldTree, newTree, patches = [], state = {}, attributesOnly = false) {
  if (!oldTree) oldTree =
  /** @type {VTree} */
  EMPTY.OBJ;
  if (!newTree) newTree =
  ...
  const isEmpty = oldTree === EMPTY.OBJ || attributesOnly; // Check for SVG in parent.
