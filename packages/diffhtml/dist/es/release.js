import { StateCache, NodeCache, ReleaseHookCache } from "./util/caches.js";
import { unprotectVTree } from "./util/memory.js";
import { Mount } from "./util/types.js";
/**
...
export default function release(domNode) {
  ...
  const state = StateCache.get(domNode); // If this was a top-level rendered element, deallocate the VTree
  // and remove the StateCache reference.

  if (state) {
    StateCache.delete(domNode); // If there is a known root association that is not in the NodeCache,
    // remove this VTree.
