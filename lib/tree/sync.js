import { NodeCache, ComponentCache } from '../util/cache';
...
const runCtor = (vTree, oldMount) => {
  ...
  ComponentCache.set(newMount, instance);
  ...
  if (oldChildNodesLength >= childNodesLength) {
    for (let i = 0; i < childNodesLength; i++) {
      if (oldChildNodes[i].nodeName !== childNodes[i].nodeName) {
        if (NodeCache.get(oldChildNodes[i])) {
          const instance = ComponentCache.get(oldChildNodes[i]);

          if (instance) {
            const vTree = childNodes[i];
            const props = Object.freeze(Object.assign({}, vTree.attributes, {
              children: Object.freeze(vTree.childNodes)
            }));
