 ...
 */
const makePropsObject = (rawAttrs, supplemental) => {
  const props = {};
...
const HTMLElement = (nodeName, rawAttrs, supplemental) => {
  ...
  if (nodeName === TOKEN) {
    const props = makePropsObject(rawAttrs, supplemental);
    return createElement(supplemental.tags.shift(), props);
  }
